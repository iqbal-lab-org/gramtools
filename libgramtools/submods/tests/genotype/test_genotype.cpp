...
class SnpsNestedInTwoHaplotypes : public ::testing::Test {
protected:
    void SetUp(){
...
TEST_F(SnpsNestedInTwoHaplotypes, MapNoReads_AllGenotypesAreNull){
    LevelGenotyper genotyper(setup.prg_info.coverage_graph, setup.coverage.grouped_allele_counts,
                             setup.read_stats, Ploidy::Haploid);
    gt_recs = genotyper.get_genotyped_records();
...
TEST_F(SnpsNestedInTwoHaplotypes, MapReads_CorrectlyGenotypedSites){
...
TEST_F(SnpsNestedInTwoHaplotypes, MapReads_CorrectlyInvalidatedSites){
    MapReadsAndHaploidGenotype();
