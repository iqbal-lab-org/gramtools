...
PRG_Info gram::submods::generate_prg_info(const marker_vec &prg_raw) {
  ...
  // Write the int vector to disk so that it can be read by sdsl for building fm
  // index
  ...
  // NB: the move is crucial here, otherwise the initialised cov_Graph's
  // destructor affects the assigned-to cov_Graph
  ...
  prg_info.prg_markers_rank =
      sdsl::rank_support_v<1>(&prg_info.prg_markers_mask);
  prg_info.prg_markers_select =
      sdsl::select_support_mcl<1>(&prg_info.prg_markers_mask);
