...
json_prg_ptr make_json_prg(gtyper_ptr const& gtyper, SegmentTracker& tracker) {
  ...
  for (auto const& site : genotyped_records) {
    ...
    json_site->set_pos(tracker.get_relative_pos(site_pos) +
...
void populate_json_prg(Json_Prg& json_prg, gtyper_ptr const& gtyper) {
  ...
  auto genotyped_records = gtyper->get_genotyped_records();
  if (!cov_graph->is_nested)
    cur_json.at("Lvl1_Sites").push_back("all");
  else {
    ...
    for (const auto& child_entry : child_m) {
      ...
      for (const auto& hapg_entry : child_entry.second) {
        ...
        cur_json.at("Child_Map")
            .at(site_index)[std::to_string(hapg_entry.first)] = JSON(copy);
      }
    }
  }
...
void add_model_specific_entries(JSON& json_site, site_entries const& entries) {
  for (auto const& entry : entries.doubles) {
    ...
    if (entry.single_val)
      json_site.at(entry.ID).push_back(entry.vals.at(0));
    else
      json_site.at(entry.ID).push_back(JSON(entry.vals));
  }
}
...
json_site_ptr make_json_site(gt_site_ptr const& gt_site) {
  ...
  for (int i{0}; i < gtype_info.alleles.size(); ++i)
    json_site.at("ALS").push_back(gtype_info.alleles.at(i).sequence);

  if (gt_site->is_null())
    json_site.at("GT").push_back(JSON::array({nullptr}));
  else
    json_site.at("GT").push_back(JSON(gtype_info.genotype));
