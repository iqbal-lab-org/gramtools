...
BuildParams commands::build::parse_parameters(
  ...
  build_description.add_options()(
      ...
      "gramtools directory")("ref",
                             po::value<std::string>(&fasta_ref)->required(),
  ...
  try {
    po::store(po::command_line_parser(opts).options(build_description).run(),
              vm);
    po::notify(vm);
