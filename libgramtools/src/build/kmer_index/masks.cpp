...
sdsl::int_vector<> gram::generate_allele_mask(const marker_vec &encoded_prg) {
  ...
  for (uint64_t i = 0; i < encoded_prg.size(); ++i) {
    ...
    if (is_site_marker) {
      current_allele_id = 1;
      if (last_allele_position >
          0) {  // Is this not the first site we are seeing?
        // Reset to 0 those positions in between the last allele marker and the
        // new variant site.
        for (int pos = last_allele_position + 1; pos < i; ++pos)
          allele_mask[pos] = 0;
      }
      continue;
    } else {
  ...
  // Reset to 0 those positions in between the last allele marker and the end of
  // the PRG string.
  for (int pos = last_allele_position + 1; pos < encoded_prg.size(); ++pos)
    allele_mask[pos] = 0;
...
sdsl::bit_vector gram::generate_prg_markers_mask(
    const marker_vec &encoded_prg) {
