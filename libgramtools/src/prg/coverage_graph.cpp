...
coverage_Node::coverage_Node(std::string const seq, int const pos,
                             int const site_ID, int const allele_ID)
    : sequence(seq),
      pos(pos),
      site_ID(site_ID),
      allele_ID(allele_ID),
      is_site_boundary(false) {
  // No need to allocate coverage if outside a variant site, as only variant
  // site coverage is used for genotyping
...
coverage_Graph::~coverage_Graph() {
  for (auto& bubble : bubble_map) {
    bubble.first->clear_edges();
    bubble.second->clear_edges();
  }
}
...
coverage_Graph::coverage_Graph(PRG_String const& vec_in) {
  ...
  par_map.empty() ? is_nested = false : is_nested = true;
}
...
void cov_Graph_Builder::setup_random_access(uint32_t const& pos) {
  ...
  if (seq_size <= 1)  // Will include all site entry and exit nodes, and
                      // sequence nodes with a single character
    random_access[pos] =
        node_access{target, 0, VariantLocus{0, ALLELE_UNKNOWN}};
  else
    random_access[pos] =
        node_access{target, seq_size - 1, VariantLocus{0, ALLELE_UNKNOWN}};
}

marker_type cov_Graph_Builder::find_marker_type(uint32_t const& pos) {
  auto const& m = linear_prg[pos];
  if (m <= 4)
    return marker_type::sequence;  // Note: the `PRG_String` constructor code
                                   // must make sure that m is > 0.

  // After passing through `PRG_String` constructor code, the only time a marker
  // is odd is when it signals a site entry.
  if (m % 2 == 1) return marker_type::site_entry;

  // Find if the allele marker, which must be in the map, is at the end position
  // or not
  auto& end_pos = end_positions.at(m);
  assert(pos <= end_pos);  // Sanity check: the allele marker must occur before,
                           // or at, the end position of the site
...
void cov_Graph_Builder::add_sequence(Marker const& m) {
  std::string c =
      decode_dna_base(m);  // Note: implicit conversion of m from uint32_t to
                           // uint8_t; this is OK because 0 < m < 5.
...
void cov_Graph_Builder::enter_site(Marker const& m) {
  ...
  if (cur_Locus.first != 0) par_map.insert(std::make_pair(m, cur_Locus));
...
void cov_Graph_Builder::end_allele(Marker const& m) {
  ...
  allele_ID++;
  cur_Node = boost::make_shared<coverage_Node>(
      coverage_Node("", cur_pos, site_ID, allele_ID));
}
...
void cov_Graph_Builder::exit_site(Marker const& m) {
  ...
  if (cur_Locus.second == FIRST_ALLELE)
    throw std::runtime_error("Site numbered " + std::to_string(m) +
                             " has only one allele");
  ...
  cur_Node = boost::make_shared<coverage_Node>(
      coverage_Node("", cur_pos, cur_Locus.first, cur_Locus.second));
}
...
covG_ptr cov_Graph_Builder::reach_allele_end(Marker const& m) {
  ...
  /* Use this instead to make coordinates reflect the LONGEST, not FIRST, allele
  in a site
...
void cov_Graph_Builder::map_targets() {
  ...
  marker_type cur_t;
  Marker cur_m;
  Marker cur_allele_ID = ALLELE_UNKNOWN;
    ...
    switch (cur_t) {
      case marker_type::sequence:
        if (prev_t != marker_type::sequence)
          random_access[pos].target = VariantLocus{
              prev_m,
              cur_allele_ID};  // Adds a target for the sequence character
        break;
      case marker_type::site_entry:
        cur_allele_ID = FIRST_ALLELE;
        if (prev_t != marker_type::sequence)
          make_site_entry_target(prev_t, prev_m, cur_m);
        break;
      case marker_type::site_end:
        if (prev_t != marker_type::sequence)
          make_site_exit_target(prev_t, prev_m, cur_m, cur_allele_ID);
        // Get the allele ID using the parental map
        if (par_map.find(cur_m - 1) != par_map.end())
          cur_allele_ID = par_map.at(cur_m - 1).second;
        else
          cur_allele_ID = ALLELE_UNKNOWN;
        break;
      case marker_type::allele_end:
        if (prev_t != marker_type::sequence)
          make_allele_end_target(prev_t, prev_m, cur_m, cur_allele_ID);
        cur_allele_ID++;
        break;
    }
    prev_m = cur_m;
...
void cov_Graph_Builder::make_site_entry_target(marker_type prev_t,
                                               Marker prev_m, Marker cur_m) {
  Marker marker_target{prev_m};
  AlleleId direct_deletion_allele{ALLELE_UNKNOWN};
  switch (prev_t) {
    case marker_type::site_entry:  // Double entry
    case marker_type::site_end:    // Adjacent sites
      break;
    case marker_type::allele_end:  // Jump to site entry
      marker_target -= 1;
      break;
  }
  std::vector<targeted_marker> new_target{
      targeted_marker{marker_target, direct_deletion_allele}};
  target_map.insert(std::make_pair(cur_m, new_target));
}

void cov_Graph_Builder::make_site_exit_target(marker_type prev_t, Marker prev_m,
                                              Marker cur_m,
                                              AlleleId cur_allele_ID) {
  Marker marker_target{prev_m};
  AlleleId direct_deletion_allele{ALLELE_UNKNOWN};
  switch (prev_t) {
    case marker_type::site_entry:  // Empty site, disallowed
      throw std::runtime_error(
          "PRG consistency error: "
          "site number " +
          std::to_string(cur_m) + " is empty");
    case marker_type::site_end:  // Double exit
      break;
    case marker_type::allele_end:  // Direct deletion, target site ID
      marker_target -= 1;
      direct_deletion_allele = cur_allele_ID;
      break;
  }
  add_exit_target(cur_m,
                  targeted_marker{marker_target, direct_deletion_allele});
}

void cov_Graph_Builder::make_allele_end_target(marker_type prev_t,
                                               Marker prev_m, Marker cur_m,
                                               AlleleId cur_allele_ID) {
  Marker marker_target{prev_m};
  AlleleId direct_deletion_allele{cur_allele_ID};
  switch (prev_t) {
    case marker_type::site_entry:  // Empty allele, direct deletion
      break;
    case marker_type::site_end:  // Double exit
      direct_deletion_allele = ALLELE_UNKNOWN;
      break;
    case marker_type::allele_end:  // Empty allele, direct deletion, target site
                                   // ID
      marker_target -= 1;
      break;
  }
...
void cov_Graph_Builder::add_exit_target(Marker cur_m, targeted_marker new_t_m) {
  if (target_map.find(cur_m) != target_map.end()) {
    ...
    std::vector<targeted_marker> new_vec{new_t_m};
...
bool operator>(const covG_ptr& lhs, const covG_ptr& rhs) {
  if (lhs->pos == rhs->pos) {
    if (lhs->site_ID == rhs->site_ID)
      ...
      return lhs->site_ID > rhs->site_ID;  // Give precedence to child bubble
  } else
...
bool compare_nodes(coverage_Node const& f, coverage_Node const& s) {
  return (f.sequence == s.sequence && f.pos == s.pos &&
          f.site_ID == s.site_ID && f.allele_ID == s.allele_ID &&
          f.coverage == s.coverage && f.is_site_boundary == s.is_site_boundary);
}
