...
PRG_String::PRG_String(std::string const &file_in, endianness en)
  ...
  if (!input)
    throw std::ios::failure("PRG String file not found");
  else {
    uint32_t c{0};
    uint8_t buffer[gram::num_bytes_per_integer];  // Note, use uint8_t and not
                                                  // char which is signed
    size_t byte_pos;
    while (true) {
      input.read((char *)&buffer,
                 gram::num_bytes_per_integer);  // Read byte by byte
      if (input.eof()) break;
  ...
  if (odd_site_end_found || en == endianness::big)
    write(output_file, endianness::little);
}
    ...
    if (is_site_marker(marker)) {
      ...
      if (seen_before) {
        ...
        throw std::runtime_error(
            ...
            " site marker " +
            std::to_string(marker) + " used for two different sites");
      } else
        seen_sites.insert(marker);
    } else {
      end_positions[marker] =
          pos;  // Inserts if does not exist, updates otherwise
    }
    ++pos;
  }
}
...
void PRG_String::write(std::string const &fname, endianness en) {
  ...
  if (en == endianness::big)
    model_byte_number = gram::num_bytes_per_integer - 1;
  else
    model_byte_number = 0;
...
std::string gram::ints_to_prg_string(std::vector<Marker> const &int_vec) {
  ...
  std::unordered_map<int, int>
      last_allele_indices;  // Will record where to close the sites.
...
marker_vec gram::prg_string_to_ints(std::string const &string_prg) {
  ...
  for (std::size_t i = 0; i < string_prg.size(); ++i) {
    ...
    switch (c) {
      ...
      default: {
        ...
        if (base == 0) {
          std::cerr << "Error: the argument " << c
                    << " is not a nucleotide char";
          exit(1);
        }
  ...
  // BOOST_LOG_TRIVIAL(info) << "Number of sites produced: " << (max_var_marker
  // -3 ) / 2;
  return encoded_prg;
}
...
uint64_t concat_marker_digits(const std::vector<int> &marker_digits) {
  uint64_t marker = 0;
  for (const auto &digit : marker_digits) marker = marker * 10 + digit;
  return marker;
}
...
void flush_marker_digits(std::vector<int> &marker_digits,
                         marker_vec &encoded_prg, uint64_t &count_chars) {
  if (marker_digits.empty()) return;
