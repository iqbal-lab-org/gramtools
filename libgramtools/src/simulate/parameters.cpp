...
SimulateParams commands::simulate::parse_parameters(
  ...
  try {
    po::store(po::command_line_parser(opts).options(simulate_description).run(),
              vm);
    po::notify(vm);
  ...
  parameters.json_out_fpath =
      full_path(output_dir_fpath, parameters.sample_id + std::string(".json"));
  parameters.fasta_out_fpath =
      full_path(output_dir_fpath, parameters.sample_id + std::string(".fasta"));
  return parameters;
}
