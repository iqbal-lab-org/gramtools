...
TEST(TargetMap, SiteEntry_ThreeCases) {
   ...
   */
  marker_vec v{5,  1, 6,  2, 6,  7, 1,  2, 8,  2,  8, 2,  9, 11, 1, 12, 3,
               12, 2, 10, 1, 10, 1, 13, 2, 14, 15, 1, 16, 2, 16, 4, 14};
...
TEST(TargetMap, SiteExit_TwoCases) {
   ...
   */
  marker_vec v{
      15, 1, 16, 2, 19, 1, 20, 2, 20,
      16, 3, 3,  7, 4,  8, 8,  1  // Direct deletion
  };
...
TEST(TargetMap, AlleleEnd_ThreeCases) {
   ...
   */
  marker_vec v{
      2, 3,  9, 2, 11, 3, 12, 3, 12, 10,
      1, 10, 4, 4, 4,  5, 6,  4, 6,  // Direct deletion
...
TEST_F(cov_G_Builder_nested, FindMarkerTypes) {
  ...
  std::array<marker_type, 5> expected{mt::site_entry, mt::allele_end,
                                      mt::sequence, mt::site_end, mt::site_end};
...
TEST_F(cov_G_Builder_nested, ParentalMap) {
  //"[A,AA,A[A,C]A]C[AC,C]G"
  // Expecting to find a single entry, for the single nested site, pointing to
  // siteID 5 & alleleID 3.
  parental_map expected{{7, VariantLocus{5, 2}}};
...
TEST_F(cov_G_Builder_nested, SiteAndAllele_IDs) {
  ...
  std::vector<VariantLocus> expected{
      {5, unkn},      {5, first}, {5, unkn},      {5, first + 1},
      {5, first + 1}, {5, unkn},  {5, first + 2}, {7, unkn},
      {7, first},     {7, unkn},  {7, first + 1}, {7, unkn},
      {5, first + 2}, {5, unkn},  {0, unkn},      {9, unkn},
      {9, first},     {9, first}, {9, unkn},      {9, first + 1},
      {9, unkn},      {0, unkn}};
...
TEST_F(cov_G_Builder_nested, NodeSizes) {
  //"[A,AA,A[A,C]A]C[AC,C]G"
  auto const &rand_access = c.random_access;
  // This test queries UNIQUE nodes, so we will skip "," which point to bubble
  // start node, and sequence continuation for nodes with size > 1
  std::vector<int> expected{0, 1, 2, 1, 0, 1, 1, 0, 1, 0, 1, 0, 2, 1, 0, 1};
  ...
  for (auto const &s : rand_access) {
    ...
    if (c.bubble_map.find(s.node) != c.bubble_map.end()) {
      if (seen_entries.find(s.node->get_site_ID()) != seen_entries.end())
        ...
        seen_entries.insert(s.node->get_site_ID());
    }
    if (s.node == prev) continue;
    auto sequence_size = s.node->get_sequence_size();

    // Test there is as much allocated per base coverage as there are characters
    // in the sequence node if we are in variant site. Outside variant sites we
    // do not genotype so do not allocate/record coverage.
    if (s.node->is_in_bubble())
      EXPECT_EQ(s.node->get_coverage_space(), sequence_size);
...
TEST_F(cov_G_Builder_nested, Bubble_Positions) {
  ...
  auto const &rand_access = c.random_access;
  // Note: allele separators (",") point to the site entry node, so we expect
  // them here
  i_v expected_site_entry_points{0, 2, 5, 7, 9, 15, 18};
  i_v expected_site_exit_points{11, 13, 20};
  ...
  for (auto const &s : rand_access) {
    ...
    try {
      ...
      EXPECT_FALSE(is_site_entry &
                   is_site_exit);  // They should not both be true
      if (is_site_entry) {
        EXPECT_TRUE(c.bubble_map.find(s.node) !=
                    c.bubble_map.end());  // The bubble is registered
        res_entries.emplace_back(pos);
      } else if (is_site_exit)
        res_exits.emplace_back(pos);
    } catch (std::out_of_range) {  // The node is not in any site; its site ID
                                   // should be 0.
      EXPECT_EQ(site_ID, 0);
      continue;
    }
...
TEST_F(cov_G_Builder_nested_adjMarkers, adjMarkerWiring) {
  ...
  EXPECT_EQ(
      entry,
      c.random_access[0].node);  // Consistent site numbering, sanity check
  ...
  EXPECT_EQ(
      entry,
      c.random_access[5].node);  // Consistent site numbering, sanity check
  auto &expected_next_entry = c.bubble_starts.at(9);
  // Expect direct edge between the site starting at index 5 and the site
  // starting at index 6
...
TEST_F(cov_G_Builder_nested_adjMarkers, targetEntries) {
  //"[A,]A[[G,A]A,C,T]"
  /**
   * First, check that nucleotide positions just after a marker target the site
   * and allele markers
   */
  marker_vec expected_site_targets = {0,  5, 0,  0, 6, 0, 0, 9, 0,
                                      10, 0, 10, 0, 8, 0, 8, 0};
  AlleleIds expected_allele_targets = {
      unkn,      first, unkn,  unkn, unkn,      unkn, unkn,      first, unkn,
      first + 1, unkn,  first, unkn, first + 1, unkn, first + 2, unkn};
  ...
  /**
   * Second, check that adjacent variant markers get correct entries in the
   * target map
   */
...
// Test for the number of sites, and that each "," character amounts to
// returning to the site entry point
TEST_F(cov_G_Builder_nested_adjMarkers, num_Bubbles) {
  ...
  std::unordered_map<Marker, int> expected{{5, 1}, {7, 2}, {9, 1}};
  ...
  for (auto const &s : c.random_access) {
    if (c.bubble_map.find(s.node) != c.bubble_map.end()) {
      ...
      // Case: at the entry node for the first time
        ...
        seen_entries.insert({s.node->get_site_ID(), 0});
    }
  }
...
TEST_F(cov_G_Builder_nested_adjMarkers, ParentalMap) {
  //"[A,]A[[G,A]A,C,T]"
  parental_map expected{{9, VariantLocus{7, first}}};
  EXPECT_EQ(c.par_map, expected);
}

namespace fs = std::filesystem;
auto const test_data_dir =
    fs::path(__FILE__).parent_path().parent_path() / "test_data";

// Make a coverage graph, serialise it to disk, reload into another coverage
// graph, and test the two are equal (provided equality has been properly
// defined).
...
TEST(Target_map, EvenIsEntry_OddIsExit) {
  ...
  std::vector<targeted_marker> targets{targeted_marker{5, ALLELE_UNKNOWN}};
  Marker seed{7};
