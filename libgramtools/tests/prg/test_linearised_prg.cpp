...
TEST(PRG_Conversion, string_to_ints2) {
  ...
  std::vector<Marker> expected = {5, 1, 1, 1, 6, 6, 1, 7, 2, 2,
                                  2, 8, 2, 2, 8, 2, 8, 6, 3};
  auto res = prg_string_to_ints(prg_string);
  EXPECT_EQ(res, expected);
}
...
TEST(PRG_Conversion, string_to_ints3) {
  ...
  std::vector<Marker> expected = {5, 1, 6, 1, 1, 6, 1, 7,  1, 8,
                                  2, 8, 1, 6, 2, 9, 1, 10, 2, 10};
  auto res = prg_string_to_ints(prg_string);
  EXPECT_EQ(res, expected);
}

/**
 * Here I want to highlight that the initial site numbering gets lost by int to
 * string conversion if the initial site numbering does not obey: 'sites entered
 * first have smaller site IDs'
 */
...
TEST(PRGString, Load_from_File) {
  /*
   * The tested file is the binary output of running `make_prg` on the following
   MSA:
                           ">R1\n"
                           "AAAAAAAAA\n"
                           ">R2\n"
  ...
  fs::path path(test_data_dir /
                "twoSegregatingClasses.fasta.max_nest10.min_match1.bin");
...
TEST(PRGString, ExitPoint_MapPositions) {
  ...
  std::unordered_map<Marker, int> expected_end_positions{{6, 9}, {8, 8}};
