...
class MockJsonSite : public Json_Site {
 public:
  ...
  MockJsonSite(allele_vec als, GtypedIndices gts, AlleleIds hapgs,
               ...
               std::size_t pos, std::string seg_id)
      : Json_Site() {
  ...
  MockJsonSite(allele_vec als, std::vector<GtypedIndices> gts,
               std::vector<AlleleIds> hapgs,
               std::vector<allele_coverages> coverages,
               std::vector<std::size_t> total_covs, std::size_t pos,
               std::string seg_id)
      : Json_Site() {
    json_site.at("ALS") = JSON(als);
    json_site.at("POS") = pos;
...
class Site_Combine_Fail : public ::testing::Test {
 protected:
  void SetUp() {
    ...
    // site: MockJsonSite sample1({"CTCCT", "CTT"}, {0, 0}, {0, 0}, {10, 2},
    // {11});
...
TEST_F(Site_Combine_Fail, GivenInconsistenHAPGs_DoesNotFail) {
  // PRGs can be ambiguous: a sequence can have multiple paths through a variant
  // site.
  // TODO This is not desirable, as it amounts to loss of information (the
  // combined allele will get the previously known HAPG value and lose its own)
...
TEST(Site_Json_AppendEntries, GivenTwoGtedSites_CorrectAppending) {
  // MockJsonSite sample1({"CTCCT", "CTT"}, {0, 0}, {0, 0}, {10, 2}, {11}, 3,
  // "gene1"); MockJsonSite sample2({"CTCCT", "CTT"}, {1, 1}, {1, 1}, {2, 10},
  // {11}, 3, "gene1");
  JSON_data_store data;
  auto sample1 = data.site1_samples.at(0), sample2 = data.site1_samples.at(1);

  sample1->combine_with(*sample2);
  MockJsonSite expected({"CTCCT", "CTT"}, {{0, 0}, {1, 1}}, {{0, 0}, {1, 1}},
                        {{10, 2}, {2, 10}}, {11, 11}, 3, "gene1");
...
TEST(Site_Combine_Success, GivenThreeSites_CorrectCombinedSite) {
  // MockJsonSite sample1({"CTCCT", "CTT"}, {0, 0}, {0, 0}, {10, 2}, {11}, 3,
  // "gene1"); MockJsonSite sample2({"CTCCT", "CTT"}, {1, 1}, {1, 1}, {2, 10},
  // {11}, 3, "gene1"); MockJsonSite sample3({"CTCCT", "GTT"}, {0, 1}, {0, 2},
  // {5, 5}, {12}, 3, "gene1");
  JSON_data_store data;
  auto sample1 = data.site1_samples.at(0), sample2 = data.site1_samples.at(1),
  ...
  MockJsonSite expected({"CTCCT", "CTT", "GTT"}, {{0, 0}, {1, 1}, {0, 2}},
                        {{0, 0}, {1, 1}, {0, 2}},
                        {{10, 2, 0}, {2, 10, 0}, {5, 0, 5}}, {11, 11, 12}, 3,
                        "gene1");
  EXPECT_EQ(sample1->get_site(), expected.get_site());

  // Now show associativity: (sample1 + sample2) + sample 3 == sample1 +
  // (sample2 + sample3)
  JSON_data_store data_again;
  sample1 = data_again.site1_samples.at(0),
  sample2 = data_again.site1_samples.at(1),
...
class PRG_Combine_Fail : public ::testing::Test {
 protected:
  void SetUp() {
    ...
    the_prg.at("Model") = "M1";
    the_prg.at("Child_Map") = {{0, {{1, JSON::array({2, 3})}}}};
...
TEST(PRG_Combine_SampleNames, GivenNamedJSONs_CanForceOrNotForceMerge) {
  ...
  prg2.set_sample_info("Sample1",
                       "I am another sample but I was named the same. Sorry.");
...
TEST(PRG_Combine_Successs2_cpy, GivenTwoPrgs_CorrectCombined) {
   ...
   * we take a copy of each of four genotyped sites: two samples genotyped at
   * two sites.
   *
   * We need these copies because calling `combine_with` on a json_prg modifies
   * both sites (copied-to and copied-from) in-place.
   *
   * Then we test manual site combining, and that the json_prg combining did the
   * right thing.
   */
  JSON_data_store data;
