...
class GetUniqueGenotypedAlleles : public ::testing::Test {
 protected:
  void SetUp() { site.set_alleles(site_alleles); }
...
TEST_F(GetUniqueGenotypedAlleles,
       GivenRepeatedGenotype_ProducedAllelesAreNotRepeated) {
  site.set_genotype(GtypedIndices{0, 0, 1});

  auto extracted_alleles = site.get_unique_genotyped_alleles();
  allele_vector expected{site_alleles.begin(), site_alleles.end() - 1};
  EXPECT_EQ(extracted_alleles, expected);
}

TEST_F(GetUniqueGenotypedAlleles,
       GivenNotUsingMock_ProducedAllelesSameAsWithMock) {
  LevelGenotypedSite real_site;
  real_site.set_alleles(site_alleles);
  real_site.set_genotype(GtypedIndices{0, 0, 1});
...
TEST_F(GetUniqueGenotypedAlleles,
       GivenUnorderedGenotype_ProducedAllelesAreOrdered) {
  site.set_genotype(GtypedIndices{2, 0});
...
TEST(NonGenotypedHaplogroups,
     GivenGenotypedSite_CorrectNonGenotypedHaplogroups) {
  ...
  site.set_genotype(
      GtypedIndices{1, 2});  // Het call of 2 alleles in same haplogroup.
