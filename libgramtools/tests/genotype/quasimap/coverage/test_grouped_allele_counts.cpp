...
TEST(GroupedAlleleCount, GivenMultipleMappings_CorrectCoverage) {
  ...
  uniqueLoci compatible_loci = {VariantLocus{7, FIRST_ALLELE},
...
TEST(GroupedAlleleCount, MultipleReads_CorrectCoverage) {
  ...
  uniqueLoci read1_compatible_loci = {VariantLocus{7, FIRST_ALLELE + 1},
                                      VariantLocus{5, FIRST_ALLELE + 2},
                                      VariantLocus{5, FIRST_ALLELE}

  };
  uniqueLoci read2_compatible_loci = {VariantLocus{7, FIRST_ALLELE + 1},
                                      VariantLocus{5, FIRST_ALLELE + 3},
                                      VariantLocus{5, FIRST_ALLELE}

  };

  coverage::record::grouped_allele_counts(coverage, read1_compatible_loci);
  coverage::record::grouped_allele_counts(coverage, read2_compatible_loci);

  auto result = coverage.grouped_allele_counts;
  SitesGroupedAlleleCounts expected = {
      GroupedAlleleCounts{{AlleleIds{0, 2}, 1}, {AlleleIds{0, 3}, 1}},
...
TEST(GroupedAlleleCount, GivenSitesGroupedAlleleCounts_CorrectHashing) {
  ...
  // Test group IDs are distinct and 'full': allocated from 0 & increasing by
  // one
...
TEST(GroupedAlleleCount_IDToCount, OneSite_CorrectGroupIDToCounts) {
  SitesGroupedAlleleCounts sites = {{{AlleleIds{0, 1}, 19}, {AlleleIds{0}, 2}}};
...
TEST(GroupedAlleleCount_IDToCount, TwoSites_CorrectGroupIDToCounts) {
  SitesGroupedAlleleCounts sites = {
      {{AlleleIds{1, 3}, 1}, {AlleleIds{1, 4}, 2}},
      {
          {AlleleIds{2}, 10},
          {AlleleIds{3, 4}, 2},
          {AlleleIds{1, 3}, 20},
...
TEST(ReverseAlleleGroupHash, Succeeds) {
  AlleleGroupHash allele_ids_groups_hash = {{AlleleIds{1, 3}, 42},
                                            {AlleleIds{1, 4}, 43}};
...
TEST(ReverseAlleleGroupHash, IsOrderedByNumericValue) {
  ...
  AlleleGroupHash allele_ids_groups_hash = {{AlleleIds{1, 3}, 30},
                                            {AlleleIds{1, 4}, 9}};
  GroupIDToAlleles reversed = get_group_id_alleles(allele_ids_groups_hash);
  GroupIDToAlleles expected{
      {"9", AlleleIds{1, 4}},
 ...
 * the groups to be listed in increasing integer order, both for `site_counts`
 * (for each site) and for `allele_groups`.
 */
class TestGetJSON : public ::testing::Test {
 protected:
  void SetUp() {
    site1 = {{AlleleIds{1, 3}, 1}, {AlleleIds{1, 4}, 2}};
    site2 = {{AlleleIds{0}, 19}, {AlleleIds{1, 4}, 5}};
...
TEST_F(TestGetJSON, SiteOne_CorrectJson) {
  ...
  auto result = get_json(sites, group_ids);
  auto result_site_counts =
      result.at("grouped_allele_counts").at("site_counts").dump();
...
TEST_F(TestGetJSON, TwoSites_CorrectFullJson) {
  ...
  std::string expected =
      std::string(R"({"grouped_allele_counts":{"allele_groups":)") +
      expected_allele_groups + std::string(R"(,"site_counts":)") +
      expected_all_counts + std::string("}}");
  EXPECT_EQ(result, expected);
}
