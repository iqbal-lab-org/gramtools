...
TEST(ReadProcessingStats,
     GivenOneOKAndOneEmptyGenomicRead_CorrectReadProcessingStats) {
  GenomicRead_vector reads{
...
/**
 * NOTE: Because coverage is 'propagated upwards', we only want to record
 * mapping statistics on level 1 sites (sites not nested in any others)
 */
TEST(ReadMappingStats, GivenCovStructAndParMap_CorrectMappingRelatedStats) {
  ReadStats stats;
  Coverage cov{{},
               SitesGroupedAlleleCounts{GroupedAlleleCounts{{AlleleIds{0}, 20}},
                                        GroupedAlleleCounts{{AlleleIds{1}, 2}},
                                        GroupedAlleleCounts{}},
               {}};
  // Here we are saying that site with index 1 is nested within site with index
  // 0
  parental_map par_map{{7, VariantLocus{5, 2}}};
  stats.compute_coverage_depth(cov, par_map);

  EXPECT_FLOAT_EQ(stats.get_mean_cov(), 10);  // mean of 20 and 0
  EXPECT_EQ(stats.get_num_sites_noCov(),
            1);  // The site with index 2 has no cov
  EXPECT_EQ(stats.get_num_sites_total(),
            2);  // There are two level 1 sites and one nested one.
}

/**
 * We can also test cov stats this (integration) way. It is more costly, but it
 * relies on proper mapping, coverage recording and parental map formation and
 * thus also tests those.
 */
