...
std::string mkdir(std::string const& parent_dirpath,
                  std::string const& child_dirpath);

// The number of bytes to use for each integer going on disk representing PRG
// string `Marker`s
 ...
 * Used for either:
 * * serialising all the necessary information for vBWT mapping to a given prg
 * after `build` process.
...
void fill_common_parameters(CommonParameters& parameters,
