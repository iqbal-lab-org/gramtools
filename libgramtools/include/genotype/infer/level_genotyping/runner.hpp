 ...
 *
 * Although a PRG moves beyond a single reference genome, each site's first
 * allele will consistently be the first allele (haplogroup) of the bubble in
 * the graph, so that we can use that as REF if needed.
 */
...
class LevelGenotyper : public Genotyper {
 ...
 public:
   ...
   */
  AlleleIds get_haplogroups_with_sites(Marker const& site_ID,
                                       AlleleIds candidate_haplogroups) const;

  void invalidate_if_needed(Marker const& parent_site_ID,
                            AlleleIds haplogroups);
  void run_invalidation_process(lvlgt_site_ptr const& genotyped_site,
                                Marker const& site_ID);

  static likelihood_related_stats make_l_stats(double mean_cov, double var_cov,
                                               double mean_pb_error);
  static CovCount find_minimum_non_error_cov(double mean_pb_error, pmf_ptr pmf);
  std::vector<double> static get_gtconf_distrib(
