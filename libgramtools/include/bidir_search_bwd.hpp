...
void process_matches_overlapping_variants(std::vector<int> &allele_empty, SA_Intervals &sa_intervals,
                                          SA_Intervals::iterator &sa_interval_it, Sites &sites,
...
void update_sites_crossed_by_reads(SA_Intervals &sa_intervals, SA_Intervals::iterator &sa_interval_it, uint64_t left_new,
                                   uint64_t right_new, std::vector<int> &allele_empty, Sites &sites,
                                   Sites::iterator &sites_it, bool &second_to_last, bool ignore, bool last,
                                   uint64_t &last_begin, const std::vector<int> &allele_mask, const bool &delete_first,
                                   const uint64_t marker, const uint64_t marker_idx, const FM_Index &fm_index);
