...
struct seq_file_struct {
  ...
  int (*origreadfunc)(seq_file_t *sf,
                      read_t *r);  // used when read = _seq_read_pop
};
...
#define seq_read_init                                                    \
  {                                                                      \
    .name = {.b = NULL, .end = 0, .size = 0},                            \
    .seq = {.b = NULL, .end = 0, .size = 0},                             \
    .qual = {.b = NULL, .end = 0, .size = 0}, .bam = NULL, .next = NULL, \
    .from_sam = false                                                    \
  }
...
static inline int seq_read_primary(seq_file_t *sf, read_t *r) {
  ...
  if (s > 0 && r->from_sam) {
    while (s > 0 &&
           seq_read_bam(r)->core.flag & (BAM_FSECONDARY | BAM_FSUPPLEMENTARY))
      s = seq_read(sf, r);
  }
#endif
  return s;
...
static inline int _seq_read_sam(seq_file_t *sf, read_t *r) {
  ...
  if (sam_read1((samFile *)sf->hts_file, (bam_hdr_t *)sf->bam_hdr,
                (bam1_t *)r->bam) < 0)
    return 0;
...
#define _func_read_unknown(_read_unknown, __getc, __ungetc, __skipline, \
                           __fastq, __fasta, __plain)                   \
  static inline int _read_unknown(seq_file_t *sf, read_t *r) {          \
    ...
    if (c == -1) return 0;                                              \
    if (c == '@') {                                                     \
      sf->format = SEQ_FMT_FASTQ;                                       \
      sf->origreadfunc = __fastq;                                       \
    } else if (c == '>') {                                              \
      sf->format = SEQ_FMT_FASTA;                                       \
      sf->origreadfunc = __fasta;                                       \
    } else {                                                            \
      sf->format = SEQ_FMT_PLAIN;                                       \
      sf->origreadfunc = __plain;                                       \
    }                                                                   \
    __ungetc(sf, c);                                                    \
...
#define _sf_gzreadline(sf, buf) \
  gzreadline((sf)->gz_file, &(buf).b, &(buf).end, &(buf).size)
#define _sf_gzreadline_buf(sf, buf) \
  gzreadline_buf((sf)->gz_file, &(sf)->in, &(buf).b, &(buf).end, &(buf).size)
#define _sf_freadline(sf, buf) \
  freadline((sf)->f_file, &(buf).b, &(buf).end, &(buf).size)
#define _sf_freadline_buf(sf, buf) \
  freadline_buf((sf)->f_file, &(sf)->in, &(buf).b, &(buf).end, &(buf).size)
...
_func_read_fastq(_seq_read_fastq_f, _sf_fgetc, _sf_fungetc, _sf_freadline)
    ...
    _func_read_fastq(_seq_read_fastq_gz, _sf_gzgetc, _sf_gzungetc,
                     _sf_gzreadline)

        _func_read_fastq(_seq_read_fastq_f_buf, _sf_fgetc_buf, _sf_fungetc_buf,
                         _sf_freadline_buf)

            _func_read_fastq(_seq_read_fastq_gz_buf, _sf_gzgetc_buf,
                             _sf_gzungetc_buf, _sf_gzreadline_buf)

    // Read FASTA
    _func_read_fasta(_seq_read_fasta_f, _sf_fgetc, _sf_fungetc, _sf_freadline)

        _func_read_fasta(_seq_read_fasta_gz, _sf_gzgetc, _sf_gzungetc,
                         _sf_gzreadline)

            _func_read_fasta(_seq_read_fasta_f_buf, _sf_fgetc_buf,
                             _sf_fungetc_buf, _sf_freadline_buf)

                _func_read_fasta(_seq_read_fasta_gz_buf, _sf_gzgetc_buf,
                                 _sf_gzungetc_buf, _sf_gzreadline_buf)

    // Read plain
    _func_read_plain(_seq_read_plain_f, _sf_fgetc, _sf_freadline, _sf_fskipline)

        _func_read_plain(_seq_read_plain_gz, _sf_gzgetc, _sf_gzreadline,
                         _sf_gzskipline)

            _func_read_plain(_seq_read_plain_f_buf, _sf_fgetc_buf,
                             _sf_freadline_buf, _sf_fskipline_buf)

                _func_read_plain(_seq_read_plain_gz_buf, _sf_gzgetc_buf,
                                 _sf_gzreadline_buf, _sf_gzskipline_buf)

    // Read first entry
    _func_read_unknown(_seq_read_unknown_f, _sf_fgetc, _sf_fungetc,
                       _sf_fskipline, _seq_read_fastq_f, _seq_read_fasta_f,
                       _seq_read_plain_f)

        _func_read_unknown(_seq_read_unknown_gz, _sf_gzgetc, _sf_gzungetc,
                           _sf_gzskipline, _seq_read_fastq_gz,
                           _seq_read_fasta_gz, _seq_read_plain_gz)

            _func_read_unknown(_seq_read_unknown_f_buf, _sf_fgetc_buf,
                               _sf_fungetc_buf, _sf_fskipline,
                               _seq_read_fastq_f_buf, _seq_read_fasta_f_buf,
                               _seq_read_plain_f_buf)

                _func_read_unknown(_seq_read_unknown_gz_buf, _sf_gzgetc_buf,
                                   _sf_gzungetc_buf, _sf_gzskipline,
                                   _seq_read_fastq_gz_buf,
                                   _seq_read_fasta_gz_buf,
                                   _seq_read_plain_gz_buf)

    // Returns 1 on success 0 if out of memory
    static inline char _seq_setup(seq_file_t *sf, bool use_zlib,
                                  size_t buf_size) {
  if (buf_size) {
    ...
    sf->origreadfunc =
        use_zlib ? _seq_read_unknown_gz_buf : _seq_read_unknown_f_buf;
  } else
    sf->origreadfunc = use_zlib ? _seq_read_unknown_gz : _seq_read_unknown_f;
  sf->readfunc = sf->origreadfunc;
  return 1;
}
...
static inline seq_format seq_guess_filetype_from_extension(const char *path) {
  ...
  const seq_format types[NUM_SEQ_EXT] = {
      ...
      SEQ_FMT_FASTA, SEQ_FMT_FASTA, SEQ_FMT_FASTA, SEQ_FMT_FASTA, SEQ_FMT_FASTA,
      SEQ_FMT_FASTA, SEQ_FMT_FASTQ, SEQ_FMT_FASTQ, SEQ_FMT_FASTQ, SEQ_FMT_FASTQ,
      SEQ_FMT_FASTQ, SEQ_FMT_FASTQ, SEQ_FMT_FASTQ, SEQ_FMT_FASTQ, SEQ_FMT_FASTQ,
      SEQ_FMT_FASTQ, SEQ_FMT_FASTQ, SEQ_FMT_PLAIN, SEQ_FMT_PLAIN, SEQ_FMT_PLAIN,
      SEQ_FMT_PLAIN, SEQ_FMT_SAM,   SEQ_FMT_BAM,   SEQ_FMT_CRAM};

  size_t extlens[NUM_SEQ_EXT];
  size_t i;
  for (i = 0; i < NUM_SEQ_EXT; i++) extlens[i] = strlen(exts[i]);

  for (i = 0; i < NUM_SEQ_EXT; i++)
    if (extlens[i] <= plen &&
        strcasecmp(path + plen - extlens[i], exts[i]) == 0)
      return types[i];
...
static inline seq_file_t *seq_open2(const char *p, bool ishts, bool use_zlib,
                                    size_t buf_size) {
  ...
  if (ishts) {
    ...
    if (hts_fmt->format == sam)
      sf->format = SEQ_FMT_SAM;
    else if (hts_fmt->format == bam)
      sf->format = SEQ_FMT_BAM;
    else if (hts_fmt->format == cram)
      sf->format = SEQ_FMT_CRAM;
    else {
...
static inline seq_file_t *seq_dopen(int fd, char ishts, bool use_zlib,
                                    size_t buf_size) {
  seq_file_t *sf = (seq_file_t *)calloc(1, sizeof(seq_file_t));
  sf->path = strdup("-");
...
static inline seq_file_t *seq_open(const char *p) {
  ...
  bool ishts =
      (fmt == SEQ_FMT_SAM || fmt == SEQ_FMT_BAM || fmt == SEQ_FMT_CRAM);
...
static inline int seq_seek_start(seq_file_t *sf)
{
  ...
  {
    fprintf(stderr, "%s failed: %s\n", sf->f_file ? "fseek" : "gzseek",
sf->path); return -1; } else if(sf->f_file == NULL && sf->gz_file == NULL) {
...
// Get min/max qual scores by reading sequences into buffer and reporting
// min/max Returns 0 if no qual scores, 1 on success, -1 if read error
...
// Returns -1 on error
// Returns 0 if not in FASTQ format/ not recognisable (offset:33, min:33,
// max:104) max_read_bases is the number of quality scores to read
static inline int seq_guess_fastq_format(seq_file_t *sf, int *minq, int *maxq) {
  ...
  *maxq = 0;
  ...
  if (*minq >= 33 && *maxq <= 73)
    return 1;  // sanger
  else if (*minq >= 33 && *maxq <= 75)
    return 5;  // Illumina 1.8+
  else if (*minq >= 67 && *maxq <= 105)
    return 4;  // Illumina 1.5+
  else if (*minq >= 64 && *maxq <= 105)
    return 3;  // Illumina 1.3+
  else if (*minq >= 59 && *maxq <= 105)
    return 2;  // Solexa
  else
    return 0;  // Unknown, assume 33 offset max value 104
}
...
#define _seq_print_wrap(fh, str, len, wrap, j, _putc) \
  do {                                                \
    size_t _i;                                        \
