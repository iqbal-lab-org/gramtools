...
#define strm_buf_init \
  { .b = NULL, .begin = 0, .end = 0, .size = 0 }

#define strm_buf_reset(sb)     \
  do {                         \
    (b)->begin = (b)->end = 1; \
  } while (0)
...
static inline StreamBuffer *strm_buf_alloc(StreamBuffer *b, size_t s) {
  b->size = (s < 16 ? 16 : s) + 1;
  if ((b->b = (char *)malloc(b->size)) == NULL) return NULL;
  b->begin = b->end = 1;
...
static inline int gzerror2(gzFile gz) {
  int e;
  gzerror(gz, &e);
  return (e < 0);
}
...
#define _func_readline(name, type_t, __gets)                      \
  static inline size_t name(type_t file, char **buf, size_t *len, \
                            size_t *size) {                       \
    if (*len + 1 >= *size) cbuf_capacity(buf, size, *len + 1);    \
    /* Don't read more than 2^32 bytes at once (gzgets limit) */  \
    size_t r = *size - *len > UINT_MAX ? UINT_MAX : *size - *len, \
           origlen = *len;                                        \
...
#define _READ_BUFFER(file, in, __read)                         \
  do {                                                         \
...
#define _func_read_buf(fname, type_t, __read)                             \
  static inline size_t fname(type_t file, void *ptr, size_t len,          \
                             StreamBuffer *in) {                          \
...
#define _func_readline_buf(fname, type_t, __read)                              \
  static inline size_t fname(type_t file, StreamBuffer *in, char **buf,        \
...
#define _func_gets_buf(fname, type_t, __read)                                  \
  static inline char *fname(type_t file, StreamBuffer *in, char *str,          \
    ...
    while (in->end > in->begin) {                                              \
      limit =                                                                  \
          (in->begin + remaining < in->end ? in->begin + remaining : in->end); \
