...
class Genotyper:
    ...
    @classmethod
    def _coverage_of_diploid_alleles(cls, allele1, allele2, allele_combination_cov, allele_groups_dict,
    ...
    def _calculate_log_likelihoods(self):
        ...
        for allele_number, per_base_cov in enumerate(self.allele_per_base_cov):
            allele_depth = Genotyper._coverage_of_one_haploid_allele(allele_number, self.allele_combination_cov,
        ...
        self.singleton_alleles_cov = Genotyper._singleton_alleles_and_coverage(self.allele_combination_cov,
                                                                               self.allele_groups_dict)
        ...
        for (allele_number1, allele_number2) in itertools.combinations(self.singleton_alleles_cov.keys(), 2):
            ...
            self.likelihoods.append(({allele_number1, allele_number2}, log_likelihood))
    ...
    def run(self):
        if len(self.allele_combination_cov) == 0 or Genotyper._total_coverage(self.allele_combination_cov) == 0:
            ...
            for allele in self.genotype:
                if allele not in self.singleton_alleles_cov:
                    self.genotype = {'.'}
                    self.genotype_confidence = 0.0
                    break
