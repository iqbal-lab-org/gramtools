...
class TestGetInferredReference(unittest.TestCase):
    ...
    def test_MultipleVcfEntries_sameGenotypeCalls(self):
        """
        Called genotype is 1 at each of the two vcf records.
        """
        # base reference: T TAT C G G
        # variants:         GCG   A
    ...
    def test_MultipleALTFields_andDifferentGenotypeCalls(self):
        """
        Called genotype in vcf file is 2 (second ALT) and 0 (REF) at two Vcf records.
        """
        # base reference: T TAT C G  GGG
        # variants:         GCG   A
        #                   G     AA
        with open(self.test_ref, "w") as ref:
            ref.write('>First_line \n TTATCGGGG')
        ...
        vcf_reader = iter([
            _MockVcfRecord(POS=2, REF="TAT", ALT=['GCG','G'], samples = [_Sample(genotype = [2])]),
            _MockVcfRecord(POS=6, REF="G", ALT=['A','AA'], samples = [_Sample(genotype = [0])]),
        ])
        discover._produce_inferred_reference(self.test_ref, vcf_reader, self.test_inferred, description = '')
    ...
    def test_HeterozygousGenotypeCall_firstGenotypeKept(self):
        # base reference: T TAT C G  GGG
        # variants:         GCG   A
        ...
        vcf_reader = iter([
            _MockVcfRecord(POS=2, REF="TAT", ALT=['GCG'], samples = [_Sample(genotype = [0, 1])]),
            _MockVcfRecord(POS=6, REF="G", ALT=['A'], samples = [_Sample(genotype = [1, 0])]),
        ])
        discover._produce_inferred_reference(self.test_ref, vcf_reader, self.test_inferred, description = '')
