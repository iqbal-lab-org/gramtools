...
def _execute_command(geno_paths, geno_report, args):
    if geno_report.get("return_value_is_0") is False:
        geno_report["gramtools_cpp_quasimap"] = {"return_value_is_0": False}
        return geno_report
    ...
    geno_report["gramtools_cpp_quasimap"] = collections.OrderedDict(
        [
            ("command", command_str),
...
def run(args):
    ...
    report._save_report(
        start_time, geno_report, _paths, command_hash_paths, _paths["report"]
    )
