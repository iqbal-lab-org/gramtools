...
def exec(fasta, ksize, nonvariant, nreads, mask, output_fpath=''):
    bls=Chain()
    bls.parseFasta(fasta)

    if output_fpath != '':
        with open(output_fpath, 'w') as kmers_fhandle:
            for i in  bls.getKmerDict(ksize,nonvariant):
                kmers_fhandle.write(i + '\n')
...
if __name__=="__main__":
    ...
    exec(options.fasta, options.ksize, options.nonvariant,
