...
Our encoding results in an alphabet of size $4+2n$ where $n$ is the number of variant sites. We used the wavelet tree to mitigate this cost from $O(|\Sigma|)$ to $O(\log(\Sigma))$. To measure how this impacts performance, we need to bear in mind how we propose the vBWT be used, although we do not fully implement it in this paper. Step 1: parse all reads, ignore non-variable positions and do not do alignment, but collect support for alternative alleles at all the sites in the PRG, and thus infer a closest reference (for a haploid) or tuple of haplotypes (for other ploidies). This step can be achieved very rapidly based on MEMs or kmer-matching using a structure like our vBWT, as demonstrated in [rapmap, sailfish, kallisto]. Step 2: full alignment to a single reference (for haploid) or tuple of partially phased haplotypes (for other ploidies). As far as we understand,  other variation graph approaches (BWBBLE, vg) focus on single-pass full alignment to the full graph rather than alignment and discovery with respect to an inferred haplotype(s), making direct comparison difficult.

We measure the speed of exact matching of 450,000 simulated 150bp error-free reads, from each of 100 random haplotypes from a PRG of  \textit{P. falciparum} chromosome 10 with only MSP3.4 variation inserted (30x depth of coverage). To avoid repeatedly calculating the same SA intervals, we precalculate a hash of the SA intervals corresponding to all 9-mers in the PRG. This one-time precalculation was done in 8 minutes using 25 threads.  We compared BWBBLE on the same dataset and, for context, measured speed of traditional mapping of reads with bwa to the haplotype from which reads were sampled. Results (mean) are displayed in the table below: 
