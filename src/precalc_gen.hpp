		...
		int k)
{
	...
	{
		std::cout << "Precalculated kmers not found, calculating them using " << THREADS << " threads\n";
		gen_precalc_kmers(csa,mask_a,kmer_fname,maxx,k);
		std::cout << "Finished precalculating kmers!\n";
	}

	std::cout << "Reading KMERS...\n";
	read_precalc_kmers(std::string(kmer_fname)+".precalc" , kmer_idx,kmer_idx_rev, kmer_sites,kmers_in_ref);
	std::cout << "DONE!\n";
}
